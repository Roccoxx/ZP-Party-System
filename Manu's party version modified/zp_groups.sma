...
new Float:fComboTime[33], Float:g_fDamageComboReward[33], g_iComboPartyHits[33], g_iComboPartyAP[33];
...
public client_putinserver( iId )
{
	...
	if(is_user_bot(iId)) g_sPlayers[ iId ][ Player_Invites ] = true;
}
...
public OnInviteMenuHandler( const iId, const iMenu, const iItem )
{
	...
	if(is_user_bot(iTarget)){
		if (!JoinGroup(iTarget, iId))
		{
			client_print_color( iTarget, print_team_default, "^4[%s]^1 No has podido unirte al grupo de^3 %s^1.", g_szPrefix, g_sPlayers[ iId ][ Player_Name ] );
			client_print_color( iId, print_team_default, "^4[%s]^3 %s^1 no pudo unirse a tu grupo.", g_szPrefix, g_sPlayers[ iTarget ][ Player_Name ] );
		}
...
AddMember( const iGroup, const iMember )
{
	...
	// COMBOLAS
	set_task(1.0, "PartyHud", iMember+TASK_HUD, _, _, "b");
}

RemoveMember( const iGroup, const iMember )
{
	/////////////////////////COMBOLAS
	remove_task(iMember+TASK_HUD);
...
public PartyHud(taskid){
	...
	static iGroup, iSize, iMember, iCount, PartyMsg[256];
	iGroup = iSize = iMember = iCount = PartyMsg[0] = 0;

	iGroup = GetGroupIndex( id ); iSize = GetGroupSize( iGroup );
	
	for(new i = 0; i < iSize; i++)
	{
		iMember = GetGroupMember( iGroup, i );
		
		if (!GetPlayerBit( g_iIsConnected, iMember ) ) continue;
		
		if(iCount) add(PartyMsg, charsmax(PartyMsg), "^n");

		format(PartyMsg, charsmax(PartyMsg), "%s%s", strlen(PartyMsg) ? PartyMsg : "^t^t^tMiembros del Party^n", g_sPlayers[ iMember ][ Player_Name ]);
		iCount++;
	}
...
{	
	...
	for ( new i = 0 ; i < iSize ; i++ )
	{
		...
		remove_task(iMember+TASK_FINISH_COMBO);
		finish_combo(iMember+TASK_FINISH_COMBO);
	}
}

public Fw_TakeDamagePre(iVictim, iInflictor, iAttacker, Float:fDamage) {
	if(GetPlayerBit(g_iIsConnected, iAttacker) && GetPlayerBit(g_iIsConnected, iVictim) && iAttacker != iVictim && GetPlayerBit(g_iHasGroup, iAttacker) && !zp_get_user_zombie(iAttacker)){
		...
		static iGroup; iGroup = GetGroupIndex( iAttacker );
		static iSize; iSize = GetGroupSize( iGroup );
		static iMember;
		...
		for(new i = 0; i < iSize; i++)
		{
			...
			g_iComboPartyHits[iMember]++; // hits totales del party

			if(bWinAP) g_iComboPartyAP[iMember]++; // asignamos un AP al combo.
		
			if(fComboTime[iMember] < gametime)
			{
				ShowPartyCombo(iMember, iAttacker, fDamage);
				fComboTime[iMember] = gametime+0.1;
			}
		}
	}
}
...
public finish_combo(taskid){
	...
	if(!GetPlayerBit(g_iIsConnected, id)) return;

	static iSize; iSize = GetGroupSize( GetGroupIndex( id ) );
	
	static recibidos; recibidos = ( g_iComboPartyAP[id] / iSize)

	if(recibidos < 0) recibidos = 0;

	zp_set_user_ammo_packs(id, zp_get_user_ammo_packs(id) + recibidos);			
	client_print_color(id, print_team_default, "^x04[PARTY]^x01 Ammopacks:^x03 %d con %d^x01 Hits en^x03 %d Personas", recibidos, g_iComboPartyHits[id], iSize);
